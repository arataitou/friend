...
//編集をつくる update 名前以外を変更出来るように edit.php
...
<html lang="en">
  ...
  <body>
	...
	<?php
		if($return == 1){
			echo '<a herf="#fakelink" class="btn btn-block btn-lg btn-danger">'.'友達が追加されました'.'</a>';
		}

		if($return == 2){
			echo '<a herf="#fakelink" class="btn btn-block btn-lg btn-danger">'.$_GET['name'].'  '.'を編集しました'.'</a>';
		}
    ...
    $name ='select * from friends_list where area_table_id = '.$_GET['id'].';';
  	$stmt = $dbh->prepare($name);
  	$stmt ->execute();
      ...
      echo '</ul>';
			      ...
			      $sei='SELECT friends_list.gender, count(*) FROM `friends_list` where area_table_id ='.$_GET['id'].' group by gender';
				  	$stmt = $dbh->prepare($sei);
				  	$stmt ->execute();
      while(1){
      					  	$rec = $stmt->fetch(PDO::FETCH_ASSOC);
      	...
      	if ($rec == false) {
                  ...
                  break;
				  	...
				  	echo $rec['gender'].'・・'.$rec['count(*)']. '人'.'</br>';
				  }


//とれたidを使ってフレンドの名前を出すそして、どこのフレンドかも出す
